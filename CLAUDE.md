...
`C:\Users\RutterAdmin\Documents\Sage 50 2026.0 SDK\.NET Samples\Sample Code`.

SSH works and is the quickest way to get at all of it — the same credentials
`release-via-ssh.sh` uses:

```bash
ssh -i ~/.ssh/tally_azure RutterAdmin@20.51.189.20 'powershell.exe -NoProfile -Command "..."'
scp -i ~/.ssh/tally_azure "RutterAdmin@20.51.189.20:C:/Program Files (x86)/Sage/Sage 50 2026.0 SDK/Sage.Peachtree.API.XML" .
```
...
user. Keep an RDP session open as `rutteradmin`, then launch the connector from
macOS with:
...
interactive-token scheduled task for `rutteradmin`, starts it, waits for the
