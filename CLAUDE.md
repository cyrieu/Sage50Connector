...
- COM uses Rutter's separate Sage-issued partner credential. The backend reads
  it from `SAGE_50_COM_USERNAME` / `SAGE_50_COM_PASSWORD`, encrypts it to a
  one-use 2048-bit RSA key generated by the connector, and never stores it on
  the Item or returns it as plaintext. The connector decrypts it in memory and
  writes `%ProgramData%\Rutter\Sage50Connector\sage-com-credential.bin` with
...
- **Production must configure the Sage COM application secret.** Set
  `SAGE_50_COM_USERNAME` and `SAGE_50_COM_PASSWORD` in the backend secret manager
  before enabling setup. `/sage-50/setup-session` fails closed with 503 when
  either value is absent, so no customer can finish with only partial access.
...
The MSI is generic: it contains no company name, connection ID, or Sage/Rutter
credential.
